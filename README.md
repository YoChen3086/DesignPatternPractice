...
- ### Chapter21:
    ### 有些類別也需要計畫生育 --> 獨體模式(Singleton)
    ###### 保證一個類別僅有一個實體，並提供一個存取他的全域訪問點。
    
- ### Chapter22:
    ### 手機軟體何時統一 --> 橋接模式(Bridge)
    ###### 將抽象部分與他的實現部分分離，使他們都可以獨立地變化。
    ###### 合成/聚合複用原則(CARP)，盡量使用合成/聚合，盡量不要使用類別繼承。
    
- ### Chapter23:
    ### 烤羊肉串引來的思考 --> 命令模式(Command)
    ###### 將一個請求封裝為一個物件，你可用不同的請求對客戶進行參數化；對請求排隊或記錄請求日誌，以及可支援可取消的操作。
    
- ### Chapter24:
    ### 加薪非要老總批? --> 職責鏈模式(Chain of Responsibility)
    ###### 使多個物件都有機會處理請求，從而避免請求的發送者和接受者之間的耦合關係。將這個物件連成一條鏈，並沿著這條鏈傳遞該請求，直到有一個物件處理它為止。
        
- ### Chapter25:
    ### 世界需要和平 --> 仲介者模式(Mediator)
    ###### 用一個仲介物件來封裝一系列的物件互動。仲介者始個物件不需要顯示地互相參考，從而使其耦合鬆散，而且可以獨立地改變他們之間的互動。
    
- ### Chapter26:
    ### 專案多也別傻傻做 --> 享元模式(Flyweight)
    ###### 運用共用技術有效地支援大量細粒度的物件。
        
- ### Chapter27:
    ### 其實你不懂老闆的心 --> 解讀器模式(Interpreter)
    ###### 給定一個語言，定義他的文法的一種表示，並定義一個解譯器，這個解譯器使用該表示來解釋語言中的句子。
    ###### 可以再加上簡單工廠模式+反射
    
- ### Chapter28:
    ### 男人和女人 --> 訪問者模式(Visitor)
    ###### 表示一個作用於某物件結構中的各元素之操作。它使你可以在不改變個元素之類別的前提下，定義作用於這些元素的新操作。
    ###### 有比較穩定的資料結構，又有易於變化的演算法的話，使用訪問者模式就是比較合適的，因為訪問者模式使得演算法操作的增加變得容易。